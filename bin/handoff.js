...
program
  ...
  .action(async (options) => {
    const spinner = ora('Initializing Handoff...').start();
    ...
    try {
      ...
      if (await fs.pathExists('.project')) {
        ...
        if (!overwrite) {
          console.log(chalk.blue('ℹ️  Handoff initialization cancelled.'));
          return;
        }
        
        await fs.remove('.project');
        spinner.start('Reinitializing Handoff...');
      }
      ...
      spinner.succeed('Handoff initialized successfully! 🎉');
      
      console.log(chalk.green('\n✅ Your project now has Handoff support!'));
...
program
  ...
  .action(async () => {
    if (!(await fs.pathExists('.project/handoff-config.md'))) {
      console.log(chalk.red('❌ No Handoff configuration found. Run "handoff init" first.'));
      return;
    }
...
program
  ...
  .action(async () => {
    if (!(await fs.pathExists('.project'))) {
      console.log(chalk.red('❌ Handoff not initialized. Run "handoff init" to get started.'));
      return;
    }
    
    console.log(chalk.green('✅ Handoff is initialized in this project'));
...
program
  ...
  .action(async () => {
    console.log(chalk.blue('Available Handoff templates:\n'));
    ...
    console.log(chalk.blue('Usage: handoff init --template <name>'));
  });
